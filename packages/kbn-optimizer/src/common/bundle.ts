...
export interface BundleSpec {
   ...
   */
  readonly entries: Array<{ pkgId: string; targets?: string[] }>;
}
...
export class Bundle {
  ...
  constructor(spec: BundleSpec) {
    ...
    this.entries = spec.entries.map((e) => ({
      pkgId: e.pkgId,
      targets: e.targets ?? [''],
    }));
...
export function parseBundles(json: string) {
  try {
    ...
    return specs.map((spec: UnknownVals<BundleSpec>): Bundle => {
      ...
      const { id, sourceRoot, outputDir, banner, entries, ...extra } = spec;
      ...
      if (!Array.isArray(entries) || !entries.every(isBundleEntry)) {
        throw new Error('`bundles[]` must have a `entries` valid property');
      }
